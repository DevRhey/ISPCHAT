...
const QueueIntegration = ({ open, onClose, integrationId }) => {
  ...
  const handleSaveDialogflow = async (values) => {
    try {
      if (values.type === 'n8n' || values.type === 'webhook' || values.type === 'typebot' || values.type === 'langgraph') values.projectName = values.name
      if (integrationId) {
                    ...
                    <Grid item xs={12} md={6} xl={6}>
                      ...
                      >
                        ...
                        >
                          ...
                          <MenuItem value="langgraph">ISPCHAT LangGraph</MenuItem>
                        </Field>
                      </FormControl>
                    </Grid>
                    {values.type === "langgraph" && (
                      <Grid xs={12} item>
                        <Paper elevation={0} style={{ padding: 12, background: "#f5f5f5" }}>
                          Motor LangGraph ISPCHAT com fluxos completos de ISP
                          (2ª via, PIX, triagem, viabilidade, OS, retenção, etc.).
                          Salve e vincule esta integração na fila.
                        </Paper>
                      </Grid>
                    )}
