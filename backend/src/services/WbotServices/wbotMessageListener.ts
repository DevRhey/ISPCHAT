...
import { startQueueFlowIfNeeded } from "../FlowServices/FlowEngine";
import langGraphListener from "../LangGraphServices/langGraphListener";
...
export const handleMessageIntegration = async (
  ...
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    await n8nService({ wbot, msg, ticket, integration: queueIntegration });
  } else if (queueIntegration.type === "typebot") {
    await typebotListener({ ticket, msg, wbot, typebot: queueIntegration });
  } else if (queueIntegration.type === "langgraph") {
    await langGraphListener({ wbot, msg, ticket, integration: queueIntegration });
  }
}
  ...
  try {
    ...
    if (whatsapp.queues.length == 1 && ticket.queue) {
      if (ticket.chatbot && !msg.key.fromMe) {
        const queueWithIntegration = await Queue.findByPk(ticket.queueId);
        if (queueWithIntegration?.integrationId) {
          const integ = await ShowQueueIntegrationService(
            queueWithIntegration.integrationId,
            companyId
          );
          if (integ?.type === "langgraph") {
            await ticket.update({
              useIntegration: true,
              integrationId: integ.id
            });
            await langGraphListener({
              wbot,
              msg,
              ticket,
              integration: integ
        ...
        const usedFlow = await startQueueFlowIfNeeded(
          ticket,
          msg,
          wbot,
          getBodyMessage
