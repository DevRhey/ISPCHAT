...
const SendWhatsAppMessage = async ({
  ...
  const number = await getContactJid(ticket.contact, ticket.isGroup, wbot);
  ...
  const sendScheduledMessagesWbot = new Queue("SendWbotMessages", connection);
  ...
  try {
    ...
    // Persiste JID real se ainda não estiver salvo
    if (!ticket.contact.remoteJid) {
      await ticket.contact.update({ remoteJid: number });
    }
